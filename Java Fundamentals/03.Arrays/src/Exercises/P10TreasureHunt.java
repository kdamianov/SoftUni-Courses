            ...
            switch (commandType) {
                ...
                case "Drop":
                    ...
                    if (position <= treasureChest.length - 1 && position>=0) {
                        String dropItems = treasureChest[position];
                        for (int i = position; i < treasureChest.length - 1; i++) {
                            treasureChest[i] = treasureChest[i +1];
                        }
                        treasureChest[treasureChest.length-1] = dropItems;
                    }else {
                        break;
                    }
                    break;
                case "Steal":
                    ...
                    if (numStolenItems >=0 && numStolenItems<= treasureChest.length) {
                        for (int i = 0; i < numStolenItems; i++) {
                            System.out.print(treasureChest[treasureChest.length - numStolenItems + i]);
                            if (i != numStolenItems - 1) {
                                System.out.print(", ");
                            }
                        }
                        String[] tempChest = new String[treasureChest.length - numStolenItems];
