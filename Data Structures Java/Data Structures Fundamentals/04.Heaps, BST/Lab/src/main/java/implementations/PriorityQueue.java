...
public class PriorityQueue<E extends Comparable<E>> implements AbstractQueue<E> {
    ...
    private boolean isLess(int first, int second) {
        return getAt(first).compareTo(getAt(second)) < 0; //връща положителна стойност, докато е по-голямо от parentIndex
    }
    ...
    public E poll() {
        ...
        Collections.swap(this.elements, 0, this.size() - 1);//разменяме местата на елементите
        this.elements.remove(this.size() - 1); //махаме този, който сме сложили на последно място
